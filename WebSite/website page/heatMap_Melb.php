...
<!-- Date: 30 Oct 2015 -->
<!-- The page is to access the melbourne data and present in a heat map-->
<html>
  <head>
        <meta charset="utf-8">
...
function initialize() {
  ...
  var mapOptions = {
    zoom: 10,
    center: new google.maps.LatLng(-37.817832, 144.973675),
...
<?php
  ...
  if(empty($date)){
      $date = 1;
      $handle = fopen("http://127.0.0.1:5984/melbourne_suburb/_design/mapreduce/_view/heatMap?key=%22$date%20Oct%202015%22","rb");
      $content = "";
      while(!feof($handle)){
...
<script>
  function getdate(str){
      ...
      window.location.href = "heatMap_Melb.php?date="+str;
  }
</script>
<br>
            ...
            <span style="font-size:26px;">Select Your Date &nbsp;&nbsp;</span>
            <?php
            if(empty($date)){
              $date = 1;
            }
